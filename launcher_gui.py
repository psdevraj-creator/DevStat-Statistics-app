...
def _detect_python() -> list[str]:
    """Auto-detect the latest Python 3.x via `py -0`, falling back to `py` or `python`."""
    try:
        ...
        if result.returncode == 0 and result.stdout.strip():
            # Parse lines like " -3.14-64 *" or " -3.12-64" and pick latest
            versions = []
            for line in result.stdout.splitlines():
                line = line.strip()
                if line.startswith("-") and "-" in line[1:]:
                    # Extract version part before -arch
                    ver = line[1:].split("-")[0].split()[0]
                    try:
                        versions.append((tuple(map(int, ver.split("."))), ver))
                    except ValueError:
                        pass
            if versions:
                versions.sort(key=lambda x: x[0], reverse=True)
                return ["py", f"-{versions[0][1]}"]
    except (OSError, subprocess.TimeoutExpired):
        pass
    # Fallback: try bare `py`, then `python`, then `python3`
                ...
                try:
                    ...
                    if self.backend_ready:
                        ...
                        if poll_count == 5:
                            ...
                            self.root.after(0, lambda: self.status_bar.configure(text="Still starting — large import..."))
                time.sleep(1)
